export default `# Terms of Service Agreement

## Introduction and Acceptance
These Terms of Service ("Terms") constitute a legally binding agreement between you ("User," "you," or "your") and AlignMate, a Philippine corporation ("AlignMate," "we," "our," or "us"), governing your use of the AlignMate posture monitoring wearable device and companion mobile application (collectively, the "Service").

By downloading, installing, or using the Service, you acknowledge that you have read, understood, and agree to be bound by these Terms and our Privacy Policy. If you do not agree to these Terms, you must not use the Service.

## Definitions

### Service Components
- **Wearable Device**: The AlignMate posture monitoring hardware device
- **Mobile Application**: The companion app for iOS and Android platforms
- **Service**: The combined hardware, software, and related services provided by AlignMate
- **Account**: Your registered user profile and associated data

### User Classifications
- **Individual User**: Person using the Service for personal health monitoring
- **Healthcare Professional**: Licensed medical practitioner using the Service for patient care
- **Enterprise User**: Organization using the Service for employee wellness programs

## Eligibility and Account Registration

### Age Requirements
- Users must be at least 18 years old or have parental/guardian consent
- Parents/guardians are responsible for minors' use of the Service
- Special protections apply to users under 18 in accordance with Philippine law

### Registration Requirements
You must provide:
- Accurate and complete personal information
- Valid email address and contact information
- Truthful health-related information relevant to posture monitoring
- Consent to data processing under our Privacy Policy

### Account Responsibilities
You are responsible for:
- Maintaining confidentiality of login credentials
- All activities under your account
- Immediately notifying us of unauthorized access
- Keeping registration information current and accurate

## Proper Use of the Service

### Intended Use
The Service is designed to:
- Monitor and analyze posture in real-time
- Provide feedback on spinal alignment and positioning
- Track posture improvement over time
- Deliver personalized recommendations for posture correction
- Support general wellness and ergonomic awareness

### Permitted Activities
You may:
- Use the Service for personal health monitoring
- Share anonymized data for research purposes (with consent)
- Export your personal data for healthcare consultations
- Customize alert settings and preferences
- Participate in community features (if available)

### Prohibited Activities
You must not:
- Use the Service for medical diagnosis or treatment decisions
- Share your account credentials with unauthorized persons
- Attempt to reverse engineer or modify the wearable device
- Interfere with the Service's security features
- Upload malicious code or engage in harmful activities
- Use the Service for any unlawful purposes
- Violate any applicable laws or regulations

## Medical Disclaimer and Limitations

### Not Medical Advice
**IMPORTANT**: The AlignMate Service is designed for wellness and ergonomic monitoring only. It is NOT a medical device and does NOT provide medical advice, diagnosis, or treatment.

### Healthcare Professional Consultation
You should consult with qualified healthcare professionals:
- Before starting any posture improvement program
- If you experience pain or discomfort
- For diagnosis or treatment of medical conditions
- Before making changes to existing medical treatments

### Limitation of Medical Claims
We make no claims that the Service will:
- Cure or treat any medical condition
- Replace professional medical care
- Provide medically accurate measurements
- Prevent injury or health problems

### User Acknowledgment
By using the Service, you acknowledge that:
- You understand the Service's limitations
- You will not rely solely on the Service for health decisions
- You will seek medical attention when appropriate
- You use the Service at your own risk

## Intellectual Property Rights

### AlignMate's Intellectual Property
We retain all rights, title, and interest in:
- The AlignMate brand, trademarks, and logos
- Software code, algorithms, and technical innovations
- User interface designs and app layouts
- Proprietary posture monitoring technology
- Documentation, training materials, and content

### User Content and Data
You retain ownership of:
- Your personal posture data and health information
- User-generated content (reviews, feedback, etc.)
- Personal profile information and preferences

### Limited License Grant
We grant you a limited, non-exclusive, non-transferable, revocable license to use the Service for its intended purpose. This license does not permit:
- Commercial use without written permission
- Redistribution or resale of the Service
- Creating derivative works based on our technology
- Removing or modifying proprietary notices

## Privacy and Data Protection

### Data Collection and Use
Your privacy is important to us. Our data practices are governed by our comprehensive Privacy Policy, which complies with the Data Privacy Act of 2012 of the Philippines.

### Key Privacy Commitments
- We collect only data necessary for Service operation
- Your health data is encrypted and securely stored
- We do not sell personal information to third parties
- You have rights to access, correct, and delete your data
- We obtain consent before using data for research

### Data Security Measures
We implement:
- Industry-standard encryption protocols
- Regular security audits and assessments
- Access controls and authentication systems
- Secure data storage and transmission methods
- Incident response procedures for data breaches

## Service Availability and Performance

### Service Provision
We strive to provide:
- Reliable device connectivity and data synchronization
- Regular software updates and improvements
- Responsive customer support
- Accurate posture monitoring within device limitations

### Service Limitations
We cannot guarantee:
- Uninterrupted Service availability
- Compatibility with all devices and operating systems
- Absolute accuracy of posture measurements
- Prevention of all technical issues or bugs

### Maintenance and Updates
We reserve the right to:
- Perform scheduled maintenance with advance notice
- Release updates that may change Service features
- Temporarily suspend Service for technical reasons
- Discontinue outdated software versions

## Payment Terms and Subscriptions

### Service Pricing
- Hardware device sold separately with one-time purchase
- Basic app features available at no additional cost
- Premium features may require subscription payment
- Pricing displayed in Philippine Pesos (PHP) or local currency

### Subscription Terms
For premium subscriptions:
- Payments processed through authorized payment providers
- Automatic renewal unless cancelled before renewal date
- Pro-rated refunds for mid-cycle cancellations
- Price changes with 30 days advance notice

### Refund Policy
- Hardware returns accepted within 30 days of purchase
- Subscription refunds available within 14 days of purchase
- Defective devices replaced under warranty terms
- Refund processing may take 5-10 business days

## Warranties and Disclaimers

### Limited Hardware Warranty
We provide a limited warranty for the wearable device:
- 1-year warranty against manufacturing defects
- Warranty void if device is damaged, modified, or misused
- Warranty covers replacement or repair at our discretion
- Warranty does not cover normal wear and tear

### Software Disclaimer
The mobile application is provided "AS IS" without warranties of any kind, including:
- Warranties of merchantability or fitness for purpose
- Warranties of accuracy or reliability
- Warranties of uninterrupted or error-free operation
- Warranties of compatibility with all devices

### Service Limitations
We disclaim responsibility for:
- Interruptions due to internet connectivity issues
- Third-party service provider failures
- User device compatibility problems
- Data loss due to user actions or device failure

## Limitation of Liability

### Scope of Limitation
To the maximum extent permitted by Philippine law, AlignMate's total liability for any claims arising from your use of the Service shall not exceed the amount you paid for the Service in the 12 months preceding the claim.

### Excluded Damages
We shall not be liable for:
- Indirect, incidental, or consequential damages
- Lost profits, revenue, or business opportunities
- Personal injury not directly caused by product defects
- Damages resulting from misuse of the Service
- Third-party actions or content

### User Assumption of Risk
By using the Service, you acknowledge and assume the risks associated with:
- Relying on posture monitoring technology
- Making health decisions based on app feedback
- Participating in recommended exercises or activities
- Sharing data through the Service

## Termination and Account Closure

### Termination by User
You may terminate your account:
- At any time through the app settings
- By contacting customer support
- Termination is effective immediately upon notice
- You remain responsible for any outstanding payments

### Termination by AlignMate
We may terminate or suspend your account:
- For violation of these Terms
- For fraudulent or illegal activity
- For prolonged inactivity (with notice)
- If required by law or court order
- For conduct harmful to other users or the Service

### Effects of Termination
Upon termination:
- Your access to the Service will cease
- Your data may be deleted in accordance with our Privacy Policy
- You must stop using the Service and wearable device
- Certain provisions of these Terms will survive termination

## Governing Law and Dispute Resolution

### Applicable Law
These Terms are governed by and construed in accordance with the laws of the Republic of the Philippines, without regard to conflict of law principles.

### Jurisdiction
Any disputes arising from these Terms or your use of the Service shall be subject to the exclusive jurisdiction of the courts of Metro Manila, Philippines.

### Alternative Dispute Resolution
Before filing any legal action, parties agree to attempt resolution through:
- Direct negotiation between the parties
- Mediation through a mutually agreed mediator
- Arbitration under Philippine Arbitration Rules (if agreed)

### Class Action Waiver
You agree to resolve disputes individually and waive any right to participate in class action lawsuits or collective proceedings.

## Modifications and Updates

### Terms Modification
We may modify these Terms at any time to:
- Reflect changes in law or regulation
- Address new features or services
- Improve clarity or enforceability
- Respond to user feedback or security concerns

### Notice of Changes
We will provide notice of material changes:
- At least 30 days before effective date
- Through email notification to registered users
- Via prominent notice in the mobile application
- By posting updated Terms on our website

### Continued Use
Your continued use of the Service after Terms modifications constitutes acceptance of the updated Terms. If you disagree with changes, you must stop using the Service.

## Compliance and Regulatory Matters

### Data Privacy Act Compliance
We comply with the Data Privacy Act of 2012 (RA 10173) including:
- Appointment of a Data Protection Officer
- Implementation of appropriate security measures
- User rights regarding personal data
- Breach notification procedures

### Health Information Standards
While not a medical device, we strive to follow relevant health information best practices:
- Secure handling of health-related data
- User consent for data processing
- Transparency in data use and sharing
- Respect for user privacy and autonomy

### Regulatory Changes
We reserve the right to modify the Service to comply with:
- New or changed laws and regulations
- Government directives or court orders
- Industry standards and best practices
- International compliance requirements

## Miscellaneous Provisions

### Entire Agreement
These Terms, together with our Privacy Policy, constitute the entire agreement between you and AlignMate regarding the Service.

### Severability
If any provision of these Terms is found invalid or unenforceable, the remaining provisions will continue in full force and effect.

### Assignment
You may not assign or transfer your rights under these Terms without our written consent. We may assign our rights and obligations without restriction.

### Force Majeure
We are not liable for delays or failures due to circumstances beyond our reasonable control, including natural disasters, government actions, or technical failures.

### Survival
Provisions regarding intellectual property, liability limitation, governing law, and dispute resolution will survive termination of these Terms.

## Contact Information

### Customer Support
For questions about these Terms or the Service:
- **Email**: support@alignmate.com
- **Phone**: +63 (2) 8XXX-XXXX
- **Website**: www.alignmate.com
- **Business Hours**: Monday-Friday, 9:00 AM - 6:00 PM (PHT)

### Legal Notices
For legal matters and compliance issues:
- **Email**: legal@alignmate.com
- **Address**: [AlignMate Legal Department Address]

### Data Protection Officer
For privacy and data protection matters:
- **Email**: privacy@alignmate.com
- **Response Time**: Within 30 days as required by law

## Acknowledgment
By using the AlignMate Service, you acknowledge that you have read, understood, and agree to be bound by these Terms of Service. You also confirm that you have the legal capacity to enter into this agreement.

*Last updated: May 28, 2025*  
*Version: 2.0*  
*Effective Date: May 28, 2025*`;
